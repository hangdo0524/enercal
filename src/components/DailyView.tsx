...
import { getSmallEnergyBgClass, getEnergyTextClass } from "../utils/colors";
    ...
    <div className="h-full flex flex-col gap-2">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-slate-200 px-3 md:px-4 py-2 md:py-3 shrink-0">
        ...
        <div className="flex items-center justify-between mb-2 pb-2 border-b border-slate-100">
          ...
          <div
            className={`text-xl md:text-2xl font-black ${getEnergyTextClass(dayData.topPct)}`}
          >
            {dayData.topPct}%
          </div>
        </div>
            ...
            {tasks.map((task) => {
              const dailyPct =
                dayData.hourlyGrid[0]?.energies.find((e) => e.task === task)
                  ?.dailyPct ?? 0;
              return (
                <div key={task} className="text-center min-w-[60px] md:min-w-[70px] flex-1">
                  <div
              ...
              <thead className="bg-slate-50 sticky top-0">
                <tr>
                  ...
                  {tasks.map((task) => (
                    <th
                      key={task}
                      className="text-center text-[10px] md:text-xs text-slate-600 font-bold py-2 md:py-3 px-0.5 md:px-1 border-b border-slate-200"
                    >
                      {task}
                    </th>
                  ))}
              ...
              <tbody>
                {filteredSlots.map((slot) => {
                  ...
                  return (
                    <tr
                      key={slot.timeSlot}
                      className={hasGood ? "bg-green-50/50" : ""}
                    >
                      ...
                      {slot.energies.map((energy) => (
                        <td
                          key={energy.task}
                          className="py-1.5 md:py-2 px-0.5 md:px-1 border-b border-slate-100"
                        >
